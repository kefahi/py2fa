...
class OTPWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        GObject.timeout_add(1000 * (runat - time.time()),
    ...
    def show_context_menu(self, widget, event):
        ...
        if event.button == 3:  # right click
            ...
            self.selected_account_name = widget.get_children()[0].get_children()[
                1].get_text()
    ...
    def add_account_dialog(self, button):
        ...
        def on_ok_clicked(widget, name_entry, secret_entry):
            ...
            if name in self.accounts:
                self.add_message.set_markup(
                    '<span color="red">Unique Name Required</span>')
            elif self.add_account(name, secret) == True:
                self.save()  # Save this new entry
                dialog.destroy()
            else:
                self.add_message.set_markup(
                    '<span color="red">Invalid Secret</span>')
